...
SELECT date(play_level_1.start_time), user_id, level, platformid, location_id, age_group, 
       sum(end_time - start_time) duration, count(session_id) sessions
  FROM play_level_1
       ...
       (SELECT id, age_group
         ...
         ON user_id = ages.id
 GROUP BY date(play_level_1.start_time), user_id, level, platformid, location_id, age_group;